<template>
    <div >
        ...
        <template #thead>
            ...
            <table-head sortable="sessions" @sorting="loadData">Количество посещений</table-head>
            <table-head sortable="validityPeriod" @sorting="loadData">Срок действия</table-head>
            <table-head sortable="price" @sorting="loadData">Цена</table-head>
            <table-head style="width: 15%">
                <router-link
                    :to="{name: 'dashboard.seasontickets.add'}"
                    tag="button" type="button" class="btn btn-success" >Добавить</router-link>
            </table-head>
        </template>

        <template #tbody="{row}">
            <table-body v-text="row.service.name"/>
            <table-body v-text="row.sessions"/>
            <table-body v-text="row.validityPeriod"/>
            <table-body v-text="row.price"/>
            <table-body :data-id="row.seasonTicketId" style="width: 15%">
  ...
  const Paginated = defineComponent({
      ...
      setup() {
          ...
          const editClick = async (e) => {
            let id = getId(e);
            await router.push({name: "dashboard.seasontickets.edit", params: {id: id}});
          }
