<template>
    <div >
        ...
        <template #thead>
            ...
            <table-head sortable="FIO" @sorting="loadData">ФИО</table-head>
            <table-head sortable="branch" @sorting="loadData">Филиал</table-head>
            <table-head>
                <router-link
                    :to="{name: 'dashboard.masters.add'}"
        ...
        <template #tbody="{row}">
            ...
            <table-body v-text="row.fio"/>
            <table-body v-text="row.address"/>
            
            <table-body :data-id="row.masterId">
  ...
  const Paginated = defineComponent({
      ...
      setup() {
          ...
          const editClick = async (e) => {
            let id = getId(e);
            await router.push({name: "dashboard.masters.edit", params: {id: id}});
          }
          ...
          const loadData = async (query) => {
            ...
            await fetch(`https://localhost:7243/api/masters?${prms}`, {
